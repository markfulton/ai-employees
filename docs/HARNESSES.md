...
| Codex | Ready | Scheduled runs, one per routine | `codex exec "<prompt>"`, confirm against `codex --help` | The sandbox: confirm it can write in the kit folder and reach the network | `[mcp_servers.<name>]` in `~/.codex/config.toml`, then `codex mcp login <name>`; skills in `.agents/skills` |
