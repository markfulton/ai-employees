...
**What this file will not do.** It will not tell you a harness supports something I could not confirm. There are seven harnesses in these tables and I have run this kit on one of them. Everything else is marked for what it is. A row that says `unknown` is worth more to you than a row that says yes and is wrong at 07:15 on a Tuesday when nobody is awake to notice.
...
| **OpenClaw** | Expected, core to it | Probe this | Unconfirmed. Probe | Probe this specifically | None I can confirm | `unknown` on browser |
| **Hermes** | Unknown | Unknown | Unknown | Unknown | Unknown | `unknown` throughout |
| **OpenCode** | Expected, core to it | Probe this | Add a browser automation server | Depends on that server's config | None I can confirm | `expected` on files |
| **GrokBot** | Unknown | Unknown | Unknown | Unknown | Unknown | `unknown` throughout |
| **Codex** | Expected, core to it | **Check the sandbox specifically** | Add a browser automation server | Depends on that server's config | None I can confirm | `expected` on files |
| **Antigravity** | Expected, core to it | Probe this | Part of the product | Probe this specifically | None I can confirm | `expected` |
...
**Hermes.** I could not confirm this harness's tool set at all, so every row below says so. That is not a verdict on the product. It is a statement about what I was able to verify. If it reads files and runs a shell command, most of this kit works, because most of this kit is file work.
...
| GrokBot | Unknown. Probe before relying on the market sweep | `unknown` |
...
Where your harness is one of the rows above that this file cannot answer, put what you found in `## Corrections` at the bottom of this file.
