...
Eight rows, eight routines, and every id carries the `csat-` prefix so they namespace cleanly beside other Agent Employees.
...
| `csat-reply-desk` | Reply desk | Weekdays | Drafts today's replies hardest first into a dated queue file, and names the remedy where the honest answer is money. It never sends |
...
This does not weaken anything, and the reason is worth being clear about. **The prompt gate was never what stopped this kit from sending.** The two stops live inside the routines: the Employee never composes a send action, never presses a barred control, never marks a ticket read, never touches a control on a billing screen, never enters a credential, and never spends. There is no path where an approval prompt is the last thing standing between a draft and a customer. Turning off the prompt removes a question about opening a tab and writing a file. It does not add a capability.
