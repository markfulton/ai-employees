...
45 7  * * 1-5    «RUN gtm-board-standup»
...
schtasks /Create /TN "gtm-board-standup"            /SC WEEKLY  /D MON,TUE,WED,THU,FRI /ST 07:45 /TR "«GTM_ROOT»\run\gtm-board-standup.cmd"
...
Task Scheduler has a setting called **Run task as soon as possible after a scheduled start is missed.** Turn it on for all eight. The window guard makes the catch up safe, and without it a laptop that was closed at the scheduled fire gets no brief at all that day.
