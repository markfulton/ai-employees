...
**In this Vietnam variant the member's lines are Vietnamese, and nothing parsed moves.** The three headings, the card ids, the artifact paths, and the pointer line stay exactly as shown, because `CONTRACT.md` section 2.7 fixes them and `gtm-launch-step-runner` reads this file. The words you write on each line are Vietnamese, addressed to the member as `anh/chị`. A blocker line carries the blocker string verbatim from its run record, whatever language that record used, because a translated blocker is a second version of a fact. Dates in front of the member read `dd/mm/yyyy`. A filled brief has this shape, and every card in it is fictional:

```
# 05/03/2026

## Today
- C-014 | Nộp hồ sơ bản đồ doanh nghiệp, form đã điền sẵn trong tab | queue/2026-03-05-form.md

## Waiting on you
- queue/2026-03-05-email.md: ba nháp, chưa tích ô nào
- C-015 | Duyệt hoặc từ chối chi bài PR, thẻ chưa có báo giá

## Blocked
- gtm-signal-sweep, open since 26/02/2026: the review platform asked for a sign in, nothing entered
...
Show dates to the member as `dd/mm/yyyy`; keep operational dates in ISO form. Then copy the passing file verbatim to `briefs/brief-YYYY-MM-DD.md`. The dated copy is the same content, not a longer version of it.
...
{"routine":"gtm-board-standup","period":"2026-03-05",
 "start":"<ISO start timestamp>","end":"<ISO end timestamp>",
 "status":"ok",
