...
**The Vietnam floors.** When this routine's state file carries no value for a key below, write this value once and never overwrite a value that is already there. The member's later edit wins and you use whatever the file holds.

| Key | Value | What it means |
|---|---|---|
| `sent_per_segment` | `30` | Sends in the evidence window with a non null `sent_on`. Below it the verdict is `chưa đủ mẫu`, with no rate and no message rewrite |
| `replies_for_message_call` | `5` | Replies or refusals with written words, from the member's statuses and notes, before any call about the message |
| `months_of_signal` | `2` | Consecutive months that both reached the floor and read low on both, before a retirement |

`worked_fraction` has no Vietnam value. Leave it as the file holds it. These are review floors for a business selling with one or two people, not market benchmarks, and the report never calls them a market rate. A queued draft or a message the member says went out with no tick is never a send.

**Where an order closes outside the website.** A sale closed by telephone, in a chat, or on cash on delivery never reaches a website purchase event. Read the primary conversion line only from the weekly scoreboards, which carry the member's order counts with their source, and never call a segment unproductive on a website event count alone. Where the scoreboards carry no order count, the conversion cell is `n/a (no order count in the scoreboards)`. Where a holiday or a sale week sits inside the window, name it in the report and do not compare its raw counts with an ordinary month, and subtract the returned orders the scoreboards record before you call a sale week a success.

**Owner words for the verdicts.** In `strategy/icp.md` and the report, the verdicts read `chưa đủ mẫu` below the floor, `sửa câu` for a message problem, `đổi nơi tìm` for a sourcing problem, and `đề xuất cho nghỉ` for a retirement. Keep segment ids, machine keys, and `verdict_history` values in English. A retirement moves the block exactly as Step 5 says; it never deletes a segment or its evidence.
