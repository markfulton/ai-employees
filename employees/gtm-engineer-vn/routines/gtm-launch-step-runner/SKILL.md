...
**The waiting pile, in this Vietnam variant.** Before you fill a `form` card, count the cards whose `status` is `filled` and whose `done` is false. If five or more are already waiting on the member's submit, fill no new form this run: add every `form` card id you would have worked to `deferred_today[]` with the reason `five filled forms already waiting on the member`, and spend the execute share on non form cards. Each filled form still needs the member to check it against the documents and press submit, and a pile past five is where a wrong price gets submitted. A deferred card keeps its place and its source, and it is never discarded.
...
# Mỗi form dưới đây đang mở trong một tab, đã điền, chưa nộp.
# Anh/chị kiểm tra, bổ sung những mục ghi ở "left for you", rồi tự bấm nộp.
# Nộp xong thì tích vào ô. Bản tin sáng mai đọc các ô đã tích.
...
In this Vietnam variant the header comment lines and the free text after each label are Vietnamese, because the member reads them. The first line, every `- key:` label, and the tick line stay exactly as shown. Never write that a listing, article, shop, or account was submitted because its form was filled, and never quote a fee you did not read on the page this run: a fee you could not confirm is written as a question for the member.
