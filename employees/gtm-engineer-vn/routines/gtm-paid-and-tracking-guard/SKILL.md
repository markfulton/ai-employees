...
| `strategy/utm-taxonomy.md` | `## Primary conversion event`, `## Conversion source`, `## Link convention`, `## Account names`, `## Read screens`, and in this Vietnam variant `## Order book`, for its `web_self_paid_share` line only |
...
**Whether that event counts orders, in this Vietnam variant.** Read `web_self_paid_share` under `## Order book`. Above 80 percent over the last thirty days, the website is where the business closes, and the event may be described as orders. At or below it, or `n/a`, the business closes in chats, by telephone, or on delivery, and every finding you write calls the event a website event, never a count of orders or sales. Record which reading applied in `conversion_event{}` as `counts_orders: true` or `false`, so `gtm-scoreboard` reads the same answer. You never open the member's order system and you never compare its figures yourself: the order counts reach the kit through `scoreboard/manual.md`, and the scoreboard sets them beside your event.
...
| Exists, zero in the window | One finding, ranked first. Setting, expected, observed. This is the single finding most worth a member's Monday. Where `counts_orders` is false, the finding says the website event is silent and says nothing about orders, because orders there close off the website |
