...
| `strategy/utm-taxonomy.md` | `## Primary conversion event`, `## Conversion source`, `## Read screens`, `## Link convention`, `## Account names`, and in this Vietnam variant `## Order book` |
...
### Vietnam weekly reading

Write the headline, the `Moved` lines, and the three call sections in Vietnamese, addressed to the member as `anh/chị`. Keep the section headings, the metric keys, and every source bracket exactly as shown below.

**Orders come from the member, never from an event.** Read `## Order book` in `strategy/utm-taxonomy.md` for what counts as a confirmed order and what shows payment, and read the counts themselves only from the lines the member typed into `scoreboard/manual.md` for dates inside this window. Show confirmed orders, paid orders, and returns as separate rows with `scoreboard/manual.md` as their source. An order paid on delivery and not yet collected is pending, never revenue. A payment screenshot a customer sent is not a payment. Where no line covers the window, each row reads `n/a (no order count typed)`. Where `conversion_event{}` in the tracking guard's state carries `counts_orders: false`, the website event keeps its own row labelled as a website event and is never added to, compared as, or described as orders.

**The rate floor is `rate_floor`, as the counting rules above set it**, and it ships at thirty sends. Below it a channel shows its counts with the `crm/contacted.jsonl` path and the window dates, and no rate. One small week is a reason to collect more, never a reason to call a segment healthy or dead.

**A holiday or sale week is named before any call.** Where the window holds one, say so in the headline, keep the week's returns in view, and do not scale or kill a channel on that week's counts alone.

Under `Kill`, `Scale`, and `Needs you`, write one concrete action each where the numbers on this page support one. Where they do not, write `Chưa đủ dữ liệu` and name the missing source. Every figure comes from a named ledger or `scoreboard/manual.md`, with its dates, and never from an example in any other file.
