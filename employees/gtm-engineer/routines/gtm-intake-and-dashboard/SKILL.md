...
`CONTRACT.md`, `ROLE.md`, `CAPABILITIES.md`, `SCHEDULE.md`, this file's own `## Corrections`, everything under `strategy/`, `strategy/CHANGELOG.md`, `board/board.json`, `crm/contacted.jsonl`, `runlog.jsonl`, all eight `state/gtm-<id>.json`, `recipes/BROWSER-RECIPES.md`, and `recipes/<flow>.json` for existence only.
