...
/*
 * runlog.mjs
 * The only sanctioned way to append a run record to «GTM_ROOT»/runlog.jsonl.
 *
 ...
 *     node scripts/runlog.mjs --routine gtm-outreach-queue --period 2026-03-04 \
 *          --start <ISO> --end <ISO> --status ok \
 *          --output "queue/2026-03-04-email.md (4 drafts)" \
 ...
 *                   folder, then the GTM_ROOT environment variable.
 *   --log <path>    Write to this file instead of «GTM_ROOT»/runlog.jsonl.
 ...
 *    "status":"ok",
 *    "outputs":["queue/2026-03-04-email.md (4 drafts)"],
 *    "blockers":[],
 *    "notes":"follow-up interval 4 days"}
 *
 * EXIT CODES
...
// CONTRACT.md 4.1, plus skipped-paused, which the pause switch in section 5 writes.
...
const CADENCE = {
  "gtm-signal-sweep": "day",
  "gtm-board-standup": "day",
  "gtm-outreach-queue": "day",
  "gtm-launch-step-runner": "day",
  "gtm-paid-and-tracking-guard": "week",
  "gtm-scoreboard": "week",
  "gtm-intake-and-dashboard": "month",
  "gtm-icp-refresh": "month",
};

// CONTRACT.md Appendix A. A stale id fails on its first line forever and the
// member never sees an error, so catch it here.
const STALE_IDS = {
  "icp-signal-sweep": "gtm-signal-sweep",
  "gtm-list-build": "gtm-signal-sweep",
  "list-build": "gtm-signal-sweep",
  "outreach-drafts": "gtm-outreach-queue",
  "gtm-outbound-draft-queue": "gtm-outreach-queue",
  "launch-step-runner": "gtm-launch-step-runner",
  "gtm-directory-and-press": "gtm-launch-step-runner",
  "directory-and-press": "gtm-launch-step-runner",
  ...
  "board-standup": "gtm-board-standup",
};
...
function resolveRoot(opts) {
  ...
  // The script lives at «GTM_ROOT»/scripts/runlog.mjs, so its parent is the
...
function validateShape(record) {
  ...
  } else if (!CADENCE[routine] && !routine.startsWith("gtm-")) {
    problems.push("routine \"" + routine + "\" is not one of the eight and does not carry the gtm- prefix");
  }
...
function good() {
  return {
    routine: "gtm-outreach-queue",
    period: "2026-03-04",
    start: "2026-03-04T08:15:11+07:00",
    end: "2026-03-04T08:34:40+07:00",
    status: "ok",
    outputs: ["queue/2026-03-04-email.md (4 drafts)", "crm/contacted.jsonl (+10 queued)"],
    blockers: [],
...
function selftest() {
  ...
  add("weekly key on a weekly routine", (r) => { r.routine = "gtm-scoreboard"; r.period = "2026-W10"; }, false, false);
  add("monthly key on a monthly routine", (r) => { r.routine = "gtm-icp-refresh"; r.period = "2026-03"; }, false, false);
  ...
  add("stale routine id", (r) => { r.routine = "outreach-drafts"; }, true, false);
  ...
  try {
    ...
    fs.writeFileSync(tmpLog, "\uFEFF{\"routine\":\"gtm-board-standup\",\"period\":\"2026-03-03\"}");
